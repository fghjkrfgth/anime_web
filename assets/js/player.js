...
// -------------------------------------------------------------------------
// VAST 3.0 / 4.0 PRE-ROLL VIDEO AD ENGINE
// -------------------------------------------------------------------------
window.playVastPreRollAd = function(container, callback) {
    const adUrl = window.VAST_AD_URL;
    if (!adUrl || typeof adUrl !== 'string' || adUrl.trim() === "") {
        if (typeof callback === 'function') callback();
        return;
    }

    const oldOverlay = document.getElementById('vast-ad-overlay');
    if (oldOverlay) oldOverlay.remove();

    const overlay = document.createElement('div');
    overlay.id = 'vast-ad-overlay';
    overlay.className = 'absolute inset-0 z-50 bg-black flex flex-col justify-between p-4 select-none';
    overlay.innerHTML = `
        <div class="flex justify-between items-center z-10">
            <span class="bg-amber-500/20 text-amber-400 border border-amber-500/40 px-2.5 py-1 rounded text-xs font-mono font-extrabold uppercase tracking-wider">Ad • Playing</span>
            <span id="vast-ad-countdown" class="bg-black/60 text-white border border-white/10 px-3 py-1 rounded-full text-xs font-mono font-bold">Skip in 5s</span>
        </div>
        <video id="vast-ad-video" class="w-full h-full object-contain absolute inset-0 z-0" playsinline></video>
        <div class="flex justify-end items-center z-10">
            <button id="vast-ad-skip-btn" class="hidden px-5 py-2 bg-white text-black font-extrabold text-xs rounded-full shadow-lg hover:bg-slate-200 transition-all uppercase tracking-wider">
                Skip Ad &rarr;
            </button>
        </div>
    `;

    const targetContainer = container || document.getElementById('player-container') || document.body;
    targetContainer.style.position = 'relative';
    targetContainer.appendChild(overlay);

    const adVideo = overlay.querySelector('#vast-ad-video');
    const countdownEl = overlay.querySelector('#vast-ad-countdown');
    const skipBtn = overlay.querySelector('#vast-ad-skip-btn');
    ...
    fetch(adUrl)
        ...
        .then(xmlText => {
            ...
            try {
                ...
                if (mediaFiles && mediaFiles.length > 0) {
                    mediaSrc = mediaFiles[0].textContent.trim();
                }
            } catch(e) {}
            ...
            timerInterval = setInterval(() => {
                ...
                if (countdownSeconds > 0) {
                    countdownEl.innerText = `Skip in ${countdownSeconds}s`;
                } else {
        ...
        .catch(err => {
            console.warn('[VAST Engine] Failed to load VAST ad XML:', err);
            finishAd();
        });
};
